...
const dragCorner = ref(null); // 'tl','tr','bl','br','move'
...
function onMouseMove(e) {
  ...
  let { mx, my, mw, mh } = dragStart.value;
...
function onMouseUp() { dragging.value = false; dragCorner.value = null; }

function doRotate(dir) { rotation.value = (rotation.value + dir + 360) % 360; draw(); }
function doMirror(dir) {
  if (dir === 'h') mirrorH.value = !mirrorH.value;
  else mirrorV.value = !mirrorV.value;
  draw();
}
function doReset() { rotation.value = 0; mirrorH.value = false; mirrorV.value = false; resetCrop(); draw(); }
function toggleCropMode() { cropMode.value = !cropMode.value; draw(); }
...
function confirm() {
  ...
  // Step 1: draw source at full resolution
  ...
  // Step 2: render with transforms into rotated-dimension canvas
  ...
  const mirror = mirrorH.value ? 'horizontal' : mirrorV.value ? 'vertical' : 'none';
  ...
  finalCanvas.toBlob((blob) => {
    emit('done', { blob, params: transformParams });
  }, 'image/jpeg', 0.92);
}
...
<template>
  <Teleport to="body">
    <div v-if="visible" class="editor-overlay" @wheel.prevent>
      ...
      <div class="editor-panel" ref="panel">
