...
public class RabbitMQConfig {
    ...
    /** TTL 重试队列——consumer 失败时显式重投到此队列，消息级 expiration 到期死信回主队列重试；
     *  重试次数由自定义 header x-retry-count 记录（随显式重投持久化，与 broker 版本无关——
     *  实测 RabbitMQ 4.x 死信时 x-death 被重置而非合并递增，不可依赖）。
     *  TTL 走消息级 expiration（consumer 重投时设置）：队列无 x-message-ttl 参数，
     *  调整 RETRY_TTL_MS 不需要删队列（队列参数不可变——曾因 TTL 10s→30s 遗留旧队列
     *  导致声明 406 PRECONDITION_FAILED，需 rabbitmqctl delete_queue 一次） */
    ...
    /**
     * 重试队列（无消费者）：消息级 expiration 到期死信回主交换机，经主绑定回到处理队列
     * （等待室语义）。无 x-message-ttl 队列参数——TTL 由 consumer 重投时在消息上设置
     * （见 PhotoProcessingConsumer 失败分支），调整 TTL 无需删队列重建。
     */
    @Bean
    Queue retryQueue() {
        Map<String, Object> args = new HashMap<>();
